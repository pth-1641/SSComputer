...
function EditItem() {
    ...
    onSubmit: async (values) => {
      ...
      const thumbnail =
        typeof thumbnail === 'string'
          ? thumbnail
          : await uploadImages([selectedThumb]);
      ...
      const { data } = await axios.post(
        ...
        {
          ...values,
          thumbnail: typeof thumbnail === 'string' ? thumbnail : thumbnail[0],
          details: {
            gallery: itemGallery,
            basic: specs,
            reviews: {
  ...
  useEffect(() => {
    (async () => {
      ...
      const category = searchParams.get('category');
      const { data } = await axios.get(`${category}/${itemId}`);
      const copyData = { ...data.details };
      delete copyData.gallery;
      delete copyData.review;
      const specs = Object.values(copyData).flat(2);
      formik.setValues({
        title: data.title,
        price: data.price,
        brand: data.brand,
      ...
      setShortDes(data.details.review.shortDescription.title);
      setPreviewFeaturedPic(data.details.review.thumbnail);
      setDetails(
        data.details.review.features.map((x) => ({
          title: x.title,
          detail: x.detail,
          id: x._id,
        }))
